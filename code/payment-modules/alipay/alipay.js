...
var default_alipay_config = {
    ...
    , trade_create_by_buyer_return_url: '/alipay/returnurl'
    , trade_create_by_buyer_notify_url: '/alipay/notifyurl'
};
...
Alipay.prototype.route = function (app) {
    var self = this;
    //当前交易模式用到下面3个消息验证,交易状态通知，交易跳转，交易退款通知
...
Alipay.prototype.create_direct_pay_by_user = function (data, res) {
    ...
    var parameter = {
        ...
        , seller_id: this.alipay_config.partner //卖家支付宝帐户 必填
...
Alipay.prototype.refund_fastpay_by_platform_pwd_notify = function (req, res) {
    ...
    alipayNotify.verifyNotify(_POST, function (verify_result) {
        if (verify_result) {//验证成功
            ...
            var details = result_details.split("#");
            var tradenos = '';
            for (var key in details) {
                var item = details[key].split("^");
                if (item.length > 0) {
                    if (item[2] == 'SUCCESS') {
                        if (tradenos == '') {
                            tradenos = item[0];
            ...
            //if (tradenos.length>0) orderdao.updateRefundOrders({
            //    data:{
            //        order_nos:tradenos
            //    }
            //});
...
Alipay.prototype.create_direct_pay_by_user_notify = function (req, res) {
    ...
    alipayNotify.verifyNotify(_POST, function (verify_result) {
        if (verify_result) {//验证成功
            ...
            //交易状态
            var trade_status = _POST['trade_status'];
            //logger.log('normal').info('alipay notify,out_trade_no:' + out_trade_no + ' trade_no:'+trade_no + ' trade_status:' + trade_status);
